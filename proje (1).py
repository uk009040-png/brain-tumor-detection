...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.5],  # Single-channel normalization for grayscale
                         std=[0.5])
])
...
class BrainTumorCNN(nn.Module):
    def __init__(self):
        ...
        # ➕ Using a dummy (fake) input to calculate the dynamic Fully Connected (FC) layer size forward (224 → 28 oluyor)
    ...
    def _get_conv_output(self):
        x = torch.randn(1, 1, 224, 224)  # 1 kanal, 224x224 
        x = self.convs(x)
